...
const { generateAccessToken } = require('../helperFunctions/tokens.js')
...
const googleCallback = async (req, res) => {
    ...
    try {
        ...
        if (userInfo) {
            try {
                const responseUser = await Person.create({
                ...
                if (error.code === 11000) {
                    const user = await Person.findOne({ email: userInfo.data.email });
